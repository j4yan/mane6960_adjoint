...
for i in range(1):
    ...
    w1 = 1.1
    w2 = 0.01
    x1[0] = w2*data[0][numData-1,0] + w1*data[0][numData-numOfPoints-1, 0]
    x1[1] = x1[0]
    x1[2] = w1*data[0][numData-1,0] + w2*data[0][numData-numOfPoints-1, 0]
    x1[3] = x1[0]
    y1[0] = data[0][numData-numOfPoints-1, ivar]
    y1[1] = np.power(x1[2], slope[0]) / np.power(x1[0], slope[0]) * y1[0]
    y1[2] = y1[1]
    y1[3] = y1[0]

    numData = 4
    x2[0] = w2*data[0][numData-1,0] + w1*data[0][numData-numOfPoints-1, 0]
    x2[1] = x2[0]
    x2[2] = w1*data[0][numData-1,0] + w2*data[0][numData-numOfPoints-1, 0]
    x2[3] = x2[0]
    y2[0] = data[0][numData-numOfPoints-1, ivar+2]
    y2[1] = np.power(x2[2], slope[1]) / np.power(x2[0], slope[1]) * y2[0]
    y2[2] = y2[1]
    y2[3] = y2[0]


    plt_t[i] = plt.plot(x1, y1, "k")
    plt_t[i] = plt.plot(x2, y2, "k")
    #  plt_t[i] = plt.plot(x2, y3, "k")

plt.annotate('2:1', xy=(0.425, 0.555), xycoords="axes fraction", fontsize=22)
plt.annotate('4:1', xy=(0.425, 0.255), xycoords="axes fraction", fontsize=22)
