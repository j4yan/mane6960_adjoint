...
including mode `d`.
"""
function getFilterOperator{T}(sbp::LineSegSBP{T}; d::Int=sbp.degree-1)
...
"""
function calcShockSensor{T,Tsol}(sbp::LineSegSBP{T}, jac::AbstractArray{T,1},
...
"""
function calcArtificialViscosity!{T,Tsol}(solver::EulerSolver{T},
                                          q::AbstractArray{Tsol,3},
                                          vis::AbstractArray{Tsol,2})
...
"""
    addLaplacianArtificialViscosity!(solver, area, q, res)

Adds the Laplacian AV of Persson and Peraire (AIAA, 2006) to the given residual.
"""
function addLaplacianArtificialViscosity!{
  T,Tarea,Tsol,Tres}(solver::EulerSolver{T}, area::AbstractArray{Tarea,2},
  ...
  for (findex, face) in enumerate(solver.bfaces)
    ...
    # get solution difference for last term in SIPG, and apply transposed operator
    for j = 1:3
      iflux[j,1] = -(solver.sbpface.normal[face.face]*
                     (qL[j,1] - qR[j,1]))
    end
    boundaryFaceIntegrate!(solver.sbpface, face.face, iflux,
