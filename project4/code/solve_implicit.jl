...
function calcStateJacobian(solver::EulerSolver{Float64},
  ...
  qc = convert(Array{Complex128,3}, q)
  resc = zeros(qc)
  ceps = 1e-30
...
Compute the Jacobian w.r.t. the state using coloring and the complex-step
method.  The residual is linearized based on the array `q`.
"""
...
"""
function solveNewton!{T,Tarea,Tsol}(solver::EulerSolver{T},
...
"""
function solveHomotopy!{T,Tarea,Tsol}(solver::EulerSolver{T},
                                      area::AbstractArray{Tarea,2},
                                      q::AbstractArray{Tsol,3};
                                      maxouter::Int=100,
                                      maxinner::Int=100,
  ...
  for k = 1:maxouter
    ...
    for j = 1:maxinner
      ...
      Base.LinAlg.BLAS.axpy!(1.0, dq, q)
    end    

    # predictor step
    calcWeakResidual!(solver, area, q, rhs, nu=0.0)
    scale!(rhs, -1.0)
    addLaplacianArtificialViscosity!(solver, area, q, rhs, nu=1.0, nu_only=true)
    Jac = calcStateJacobian(solver, area, q, nu=nu)
    rhsvec = vec(rhs)
    solvec = -sparse(Jac)\rhsvec
    tangent_old[:,:,:] = tangent[:,:,:]
    ...
    scale!(tangent, fac)
    Base.LinAlg.BLAS.axpy!(-alpha, tangent, q)
...
"""
function calcCNWeakResidual!{T,Tsrc,Tsol,Tres}(solver::EulerSolver{T}, dt::T,
...
"""
    stepCN!(solver, area, q, dt[, numiter=100, tol=1e-10, display=false])
...
relative tolerance to which the problem is converged, and `numiter` defines the
maximum number of iterations.  Setting `display=true` will print details
regarding the convergence history.
"""
function stepCN!{T,Tsrc,Tsol}(solver::EulerSolver{T},
                              src::AbstractArray{Tsrc,1},
...
"""
function unsteadySolveCN!{T,Tsrc,Tsol}(solver::EulerSolver{T},
                                       src::AbstractArray{Tsrc,1},
                                       q::AbstractArray{Tsol,3},
                                       time::T, nsteps::Int;
                                       display::Bool=false,
